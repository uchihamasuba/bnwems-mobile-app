...
class LeaderPaymentEvidenceUploadScreen extends StatefulWidget {
  ...
  @override
  State<LeaderPaymentEvidenceUploadScreen> createState() => _LeaderPaymentEvidenceUploadScreenState();
}

class _LeaderPaymentEvidenceUploadScreenState extends State<LeaderPaymentEvidenceUploadScreen> {
  ...
  final _noteController = TextEditingController(text: 'Khách hàng chuyển khoản cọc online thành công.');
    ...
    if (_formKey.currentState!.validate()) {
      ...
      Future.delayed(const Duration(milliseconds: 700), () {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Minh chứng thanh toán đã được gửi Manager xác nhận!')),
        );
              ...
              child: Form(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const Text('Thông tin thanh toán:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                    const SizedBox(height: 6),
                    _buildPaymentInfoCard(),
                    AppSizes.spacingL,

                    const Text('Ảnh chụp biên lai chuyển khoản:', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 13)),
                    const SizedBox(height: 6),
                    _buildEvidenceCard(),
      ...
      child: Column(
        children: [
          ...
          TextFormField(
            ...
            decoration: const InputDecoration(labelText: 'Số tiền nộp (VNĐ)', prefixText: 'đ '),
            validator: (val) => val == null || val.isEmpty ? 'Không để trống số tiền' : null,
          ),
          const SizedBox(height: AppSizes.m),
          DropdownButtonFormField<String>(
            isExpanded: true,
  ...
  Widget _buildEvidenceCard() {
    return InfoCard(
      child: InkWell(
        onTap: () {
          setState(() {
            _evidencePhoto = 'evidence_receipt_uploaded_${DateTime.now().millisecondsSinceEpoch}';
          });
        },
        child: Container(
          height: 120,
          ...
          child: Center(
            child: Column(
              ...
              children: [
                const Icon(Icons.document_scanner_outlined, color: AppColors.primary, size: 28),
                const SizedBox(height: 6),
                Text(
                  _evidencePhoto,
